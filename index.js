...
(function() {
  var Argument, BranchPattern, Command, Dict, DocoptExit, DocoptLanguageError, Either, LeafPattern, OneOrMore, Option, Optional, OptionsShortcut, Pattern, Required, Tokens, _split, any, docopt, endsWith, enumerate, extras, formal_usage, isInteger, isUpper, parse_argv, parse_atom, parse_defaults, parse_expr, parse_long, parse_pattern, parse_section, parse_seq, parse_shorts, partition, print, startsWith, transform, zip,
  ...
  startsWith = String.prototype.startsWith ? function(string, searchString, position) {
  ...
  endsWith = String.prototype.endsWith ? function(string, searchString, position) {
  ...
  _split = function(string) {
    return string.trim().split(/\s+/).filter(function(i) {
      return i !== '';
    });
  };
  ...
  LeafPattern = (function(superClass) {
    ...
    LeafPattern.prototype.match = function(left, collected) {
      ...
      if (isInteger(this.value) || this.value instanceof Array) {
  ...
  Option = (function(superClass) {
    ...
    Option.parse = function(option_description) {
      ...
      ref1 = partition(option_description.trim(), '  '), options = ref1[0], _ = ref1[1], description = ref1[2];
  ...
  parse_shorts = function(tokens, options) {
    ...
    token = tokens.move();
    console.assert(startsWith(token, '-') && !startsWith(token, '--'));
  ...
  parse_long = function(tokens, options) {
    ...
    ref = partition(tokens.move(), '='), long = ref[0], eq = ref[1], value = ref[2];
    ...
    if (tokens.error === DocoptExit && similar.length === 0) {
      similar = (function() {
        ...
        for (j = 0, len = options.length; j < len; j++) {
          o = options[j];
          if (o.long && startsWith(o.long, long)) {
            results.push(o);
          }
        }
        return results;
  ...
  parse_atom = function(tokens, options) {
    ...
    } else if (startsWith(token, '<') && endsWith(token, '>') || isUpper(token)) {
  ...
  parse_defaults = function(doc) {
    ...
    for (j = 0, len = ref.length; j < len; j++) {
      s = ref[j];
      ref1 = partition(s, ':'), _ = ref1[0], _ = ref1[1], s = ref1[2];
      split = ('\n' + s).split(new RegExp('\\n[ \\t]*(-\\S+?)')).slice(1);
      odd = (function() {
        var len1, q, results;
        results = [];
  ...
  formal_usage = function(section) {
    var _, pu, ref, s;
    ref = partition(section, ':'), _ = ref[0], _ = ref[1], section = ref[2];
    pu = _split(section);
    return '( ' + ((function() {
      var j, len, ref1, results;
      ref1 = pu.slice(1);
